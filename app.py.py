...
with col_c:
    incluir_variacao_dre = st.checkbox(
        "Deseja levar a variação das contas de resultado (DRE) para o diagnóstico?",
        value=False,
        help="Adiciona slides extras ao PPT comparando cada conta da DRE entre períodos consecutivos, em R$ e em %.",
    )

    dre_variacao_pares = None
    comentarios_variacao = {}
    if incluir_variacao_dre:
        dre_variacao_pares = p.build_dre_variacao_pares(statements, indicators, labels)
        flagged = [(pi, li) for pi, par in enumerate(dre_variacao_pares)
                   for li, l in enumerate(par["linhas"]) if l["relevante"]]
        if flagged:
            st.markdown("**Variações relevantes na DRE (acima de 20% ou R$ 0,5 mi) — comente se quiser (opcional):**")
            for pi, li in flagged:
                par = dre_variacao_pares[pi]
                linha = par["linhas"][li]
                comment_key = f"variacao_coment_{pi}_{linha['key']}"
                sinal = "+" if linha["diff"] >= 0 else ""
                pct_txt = f"{sinal}{linha['pct']:.1f}%" if linha["pct"] == linha["pct"] else "n/d"
                desc = f"{linha['label']} — {par['label_a']} → {par['label_b']}: {sinal}{linha['diff']:.1f} mi ({pct_txt})"
                valor = st.text_area(desc, value=st.session_state.get(comment_key, ""), key=comment_key, height=60)
                comentarios_variacao[f"{pi}|{linha['key']}"] = valor
        else:
            st.caption("Nenhuma variação acima de 20% ou R$ 0,5 mi entre os períodos cobertos.")
    ...
    if st.button("Gerar apresentação PPT (diagnóstico)"):
        with st.spinner("Gerando PPT…"):
            ...
            pptx_report.build_presentation(statements, indicators, out_path, group_name=group_name,
                                            incluir_variacao_dre=incluir_variacao_dre,
                                            dre_variacao_pares=dre_variacao_pares,
                                            dre_variacao_comentarios=comentarios_variacao)
            with open(out_path, "rb") as f:
